...
class SqlAlchemyPersonRepository(PersonRepository):
    ...
    async def get_by_id(self, person_id: UUID) -> Optional[Person]:
        async with self._sessionmaker() as session:
            ...
            if not result:
                return None
            return Person(id=UUID(result.id), name=result.name, email=result.email, age=result.age)
    ...
    async def list(self) -> List[Person]:
        async with self._sessionmaker() as session:
            ...
            return [Person(id=UUID(r.id), name=r.name, email=r.email, age=r.age) for r in rows]

    async def update(self, person: Person) -> Person:
        async with self._sessionmaker() as session:
            q = sa_update(PersonModel).where(PersonModel.id == str(person.id)).values(
