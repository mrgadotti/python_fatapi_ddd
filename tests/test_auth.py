...
def test_register_and_login(client):
    ...
    resp = client.post("/auth/register", json={"email": unique_email, "password": "secret123"})
    ...
    # login
    resp2 = client.post("/auth/login", json={"email": unique_email, "password": "secret123"})
...
def test_logout_revokes_token_and_protected_endpoint(test_user, client):
    ...
    create_resp = client.post("/persons/", json={"name": "Alice", "email": "alice@example.com", "age": 30},
    ...
    create_resp2 = client.post("/persons/", json={"name": "Bob", "email": "bob@example.com", "age": 40},
                               headers={"Authorization": f"Bearer {token}"})
